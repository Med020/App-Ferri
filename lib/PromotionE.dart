...
class PProductScreenE extends StatefulWidget {
  final int id;
  const PProductScreenE({Key? key, required this.id}) : super(key: key);

  @override
  State<PProductScreenE> createState() => _PProductScreenStateE();
}

class _PProductScreenStateE extends State<PProductScreenE> {
  List<Cadre> cadres = [];
  ...
  Future<void> fetchData() async {
    try {
      final response = await http.get(
          Uri.parse('http://192.168.1.6/app_ferri/Promotions_electricite.php'));
            ...
            child: GestureDetector(
              onTap: () {
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) =>
                        ProductDetailScreenNE(product: cadres[index], id: id),
                  ),
                );
              },
